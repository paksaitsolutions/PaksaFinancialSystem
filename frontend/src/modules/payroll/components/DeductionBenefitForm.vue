...
@media screen and (max-width: 960px) {
  .p-fluid .p-field {
    ...
    <div class="field">
      <label for="amount">
        {{ formData.amountType === 'fixed' ? 'Amount' : 'Percentage' }}
      </label>
      <InputNumber
        id="amount"
        v-model="formData.amount"
    ...
    <div class="field">
      ...
      <InputSwitch id="taxable" v-model="formData.taxable" />
    </div>

    <div class="field">
      <label for="active">Active</label>
      <InputSwitch id="active" v-model="formData.active" />
    </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const rules = {
      ...
      description: {},
      amountType: { required: helpers.withMessage('Amount type is required', required) },
      amount: {
        required: helpers.withMessage('Amount is required', required),
        minValue: helpers.withMessage('Amount must be greater than 0', minValue(0.01)),
        maxValue: helpers.withMessage('Percentage cannot exceed 100', (value: number) => {
          return formData.amountType === 'fixed' || (value && value <= 100);
        }),
      },
    };
    ...
    const deductionTypes = [
      ...
      { label: 'Garnishments', value: 'garnishment' },
    ...
    // Initialize form with prop data if editing
    watch(
      () => props.deduction,
      (newVal) => {
        if (newVal) {
          Object.assign(formData, {
            type: newVal.type || 'deduction',
            name: newVal.name || '',
            description: newVal.description || '',
            amountType: newVal.amount_type || 'fixed',
            amount: newVal.amount || 0,
            taxable: newVal.taxable || false,
