from fastapi import APIRouter, Depends, HTTPException, status, Query, Request, APIRouter
...
from ....models.tax_exemption_certificate import TaxExemptionCertificate as TaxExemptionCertificateModel
...
def convert_to_schema(cert: TaxExemptionCertificateModel) -> TaxExemptionCertificateSchema:
...
# Tax Exemption Certificate Endpoints
...
@router.post(
    ...
    tags=["tax-exemption-certificates"]
)
...
@router.get(
    "/exemption-certificates/{certificate_id}",
    response_model=TaxExemptionCertificateSchema,
...
async def update_exemption_certificate(
    *,
    db: Session = Depends(get_db),
    certificate_id: UUID4,
...
@role_required(["admin"])
async def delete_exemption_certificate(
    *,
    db: Session = Depends(get_db),
    certificate_id: UUID4,
    current_user: User = Depends(get_current_active_user)
...
@router.get(
    "/exemption-certificates/customer/{customer_id}",
...
async def get_customer_exemption_certificates(
    *,
    db: Session = Depends(get_db),
    customer_id: UUID4,
    is_active: Optional[bool] = None,
