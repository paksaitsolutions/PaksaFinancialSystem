...
class ConsolidatedReportingService(BaseService):
    ...
    def generate_module_activity_report(
        ...
        # AP Module Activity
        ap_invoices = (
            self.db.query(func.count(APInvoice.id), func.sum(APInvoice.total_amount))
            .filter(
        ...
        ap_payments = (
            self.db.query(func.count(APPayment.id), func.sum(APPayment.amount))
            .filter(
        ...
        # AR Module Activity
        ar_invoices = (
            self.db.query(func.count(ARInvoice.id), func.sum(ARInvoice.total_amount))
            .filter(
        ...
        ar_payments = (
            self.db.query(func.count(ARPayment.id), func.sum(ARPayment.amount))
            .filter(
        ...
        payroll_runs = (
            self.db.query(
                func.count(PayrollRun.id), func.sum(PayrollRun.total_gross_pay)
            )
            .filter(
        ...
        bank_transactions = (
            self.db.query(
                func.count(BankTransaction.id), func.sum(BankTransaction.amount)
            )
            .filter(
        ...
        gl_entries_by_module = (
            ...
            .group_by(JournalEntry.source_module)
            .all()
        )
    ...
    def generate_account_activity_summary(
        ...
        for result in results:
            ...
            # Add module activity
            account_summary[account_type]["accounts"][account_key]["modules"][
            ...
            account_summary[account_type]["accounts"][account_key]["totals"][
                "transaction_count"
            ] += result.transaction_count
            account_summary[account_type]["accounts"][account_key]["totals"][
                "total_debits"
            ] += float(result.total_debits or 0)
            account_summary[account_type]["accounts"][account_key]["totals"][
                "total_credits"
            ] += float(result.total_credits or 0)

            # Update account type totals
            account_summary[account_type]["totals"][
                "transaction_count"
            ] += result.transaction_count
            account_summary[account_type]["totals"]["total_debits"] += float(
                result.total_debits or 0
            )
            account_summary[account_type]["totals"]["total_credits"] += float(
                result.total_credits or 0
            )
    ...
    def generate_integration_health_report(self, company_id: UUID) -> Dict[str, Any]:
        ...
        ap_without_gl = (
            self.db.query(func.count(APInvoice.id))
            .outerjoin(
                JournalEntry,
                and_(
            ...
            .filter(APInvoice.company_id == company_id, JournalEntry.id.is_(None))
            .scalar()
        )

        ar_without_gl = (
            self.db.query(func.count(ARInvoice.id))
            .outerjoin(
                JournalEntry,
                and_(
            ...
            .filter(ARInvoice.company_id == company_id, JournalEntry.id.is_(None))
            .scalar()
        )
        ...
        modules_with_gl = (
            self.db.query(JournalEntry.source_module, func.count(JournalEntry.id))
            .filter(
                JournalEntry.company_id == company_id,
                JournalEntry.source_module.isnot(None),
            )
    ...
    def _calculate_integration_health_score(
        self, ap_orphaned: int, ar_orphaned: int, integrated_modules: int
    ) -> float:
    ...
    def _generate_integration_recommendations(
        self, ap_orphaned: int, ar_orphaned: int, modules_with_gl: List
    ) -> List[str]:
        ...
        if missing_modules:
            recommendations.append(
                f"Enable GL integration for modules: {', '.join(missing_modules)}"
            )
