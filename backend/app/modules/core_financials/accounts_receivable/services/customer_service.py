...
from ..models import Customer, ARInvoice, ARPayment, CollectionActivity

class CustomerService:
    """Service for customer management operations"""
    
    async def create_customer(self, db: AsyncSession, customer_data: dict, user_id: int):
    ...
    async def get_customers(self, db: AsyncSession, skip: int = 0, limit: int = 100,
    ...
    async def update_customer(self, db: AsyncSession, customer_id: int, customer_data: dict, user_id: int):
    ...
    async def get_customer_aging_analysis(self, db: AsyncSession, customer_id: Optional[int] = None):
    ...
    async def update_credit_limit(self, db: AsyncSession, customer_id: int, credit_data: dict, user_id: int):
    ...
    async def place_credit_hold(self, db: AsyncSession, customer_id: int, hold_data: dict, user_id: int):
    ...
    async def get_customer_payment_history(self, db: AsyncSession, customer_id: int, limit: int = 50):
        """Get customer payment history"""
        query = select(ARPayment).options(
            selectinload(ARPayment.invoice_applications).selectinload(ARPayment.invoice)
        ).where(
            ARPayment.customer_id == customer_id,
            ARPayment.status == "processed"
        ).order_by(desc(ARPayment.payment_date)).limit(limit)
