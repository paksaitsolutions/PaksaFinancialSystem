<<<<<<< HEAD:backend/app/modules/core_financials/accounting/models/reconciliation.py
"""
...
class ReconciliationRule(Base):
    """
    Defines rules for automatic reconciliation of transactions.
    """
    ...
    priority: Mapped[int] = mapped_column(default=0)  # Higher priority rules are applied first
    
    # Matching criteria (supports pattern matching on various fields)
    reference_pattern: Mapped[Optional[str]] = mapped_column(String(200))
    description_pattern: Mapped[Optional[str]] = mapped_column(String(500))
    amount_min: Mapped[Optional[float]] = mapped_column(Numeric(19, 4))
    amount_max: Mapped[Optional[float]] = mapped_column(Numeric(19, 4))
    
    # Action to take when matched
    auto_approve: Mapped[bool] = mapped_column(default=False)
    category_id: Mapped[Optional[UUID]] = mapped_column(PG_UUID(as_uuid=True), ForeignKey('categories.id'))
...
class ReconciliationAuditLog(Base):
    ...
    def __repr__(self):
        return f"<ReconciliationAuditLog {self.action} by {self.user_id} at {self.created_at}>"
>>>>>>> e96df7278ce4216131b6c65d411c0723f4de7f91:backend/app/models/reconciliation.py
